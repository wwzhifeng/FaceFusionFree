...
def pre_check() -> bool:
    ...
    model_sources.pop('xseg_3', None)  #这行代码跳过 xseg_3 的源文件验证

    return True and conditional_download_sources(model_sources)  # 跳过哈希校验
...
def create_occlusion_mask(crop_vision_frame : VisionFrame) -> Mask:
    ...
    print("当前 face_occluder_model:", face_occluder_model)  # 确保它被正确获取

    if face_occluder_model is None:
        raise ValueError("错误: state_manager 没有正确存入 face_occluder_model")
...
def forward_occlude_face(prepare_vision_frame : VisionFrame) -> Mask:
    face_occluder_model = state_manager.get_item('face_occluder_model')
    print("当前 face_occluder_model:", face_occluder_model)  # 确保是 xseg_3
    print("推理池内容:", get_inference_pool().keys())  # 打印所有加载的模型，看看有没有 xseg_3
    face_occluder = get_inference_pool().get(face_occluder_model)
    if face_occluder is None:
        raise ValueError(f"错误: {face_occluder_model} 未正确加载到推理池！")
